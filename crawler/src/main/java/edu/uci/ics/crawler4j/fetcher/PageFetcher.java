...
public class PageFetcher extends Configurable {
  ...
  public PageFetchResult fetchPage(WebURL webUrl)
      throws InterruptedException, IOException, PageBiggerThanMaxSizeException, ContentFetchException {
    ...
    try {
      ...
      if (statusCode == HttpStatus.SC_MOVED_PERMANENTLY || statusCode == HttpStatus.SC_MOVED_TEMPORARILY ||
        ...
        if (fetchResult.getEntity() != null) {
          ...
          // Checking language using header only at this time
          Header language = response.getLastHeader("Content-language");
          if (language != null && !config.getAcceptedLanguages().contains(language.getValue())) {
             response.close();
